...
async def _audit(db: aiosqlite.Connection, actor: str, action: str,
                 object_type: str, object_id: str | None, details: str):
...
async def get_balance(identifier: str, as_of: str | None = None) -> dict | None:
    ...
    return {
        ...
        "type": acct["type"], "normal_side": acct["normal_side"],
        "as_of": as_of or "latest", "balance_minor": bal, "balance": _fmt(bal),
    }


async def get_account_ledger(identifier: str, date_from: str | None = None,
                             date_to: str | None = None, limit: int = 100) -> dict | None:
...
async def get_transaction(txn_id: str) -> dict | None:
    ...
    txn["total_minor"] = sum(line["amount_minor"] for line in txn["lines"] if line["direction"] == "debit")
...
async def search_transactions(q: str | None = None, date_from: str | None = None,
                              ...
                              status: str | None = None, limit: int = 25, offset: int = 0) -> dict:
    db = await get_db()
    conditions, params = ["1=1"], []
    if q:
    ...
    count = await db.execute_fetchall(f"SELECT COUNT(*) AS cnt FROM transactions t WHERE {where}", params)
...
async def trial_balance(as_of: str | None = None) -> dict:
    ...
    for r in rows:
        ...
        accounts.append({
            ...
            "debit_minor": debit_bal, "debit": _fmt(debit_bal),
            "credit_minor": credit_bal, "credit": _fmt(credit_bal),
        })
    return {
        "as_of": as_of or "latest", "accounts": accounts,
        "total_debit_minor": total_debit, "total_debit": _fmt(total_debit),
        "total_credit_minor": total_credit, "total_credit": _fmt(total_credit),
        "balanced": total_debit == total_credit,
    }


async def _type_total(account_type: str, date_from: str | None = None,
                      date_to: str | None = None) -> tuple[int, list[dict]]:
    ...
    for r in rows:
        ...
        total += bal
        items.append({"code": a["code"], "name": a["name"], "balance_minor": bal, "balance": _fmt(bal)})
    return total, items
...
async def profit_loss(date_from: str | None = None, date_to: str | None = None) -> dict:
    ...
    return {
        ...
        "income": income, "total_income_minor": income_total, "total_income": _fmt(income_total),
        "expenses": expenses, "total_expenses_minor": expense_total, "total_expenses": _fmt(expense_total),
        "net_profit_minor": net, "net_profit": _fmt(net),
    }
...
async def balance_sheet(as_of: str | None = None) -> dict:
    ...
    return {
        ...
        "assets": assets, "total_assets_minor": assets_total, "total_assets": _fmt(assets_total),
        "liabilities": liabilities, "total_liabilities_minor": liab_total, "total_liabilities": _fmt(liab_total),
        "equity": equity + [{"code": "—", "name": "Current Earnings", "balance_minor": earnings, "balance": _fmt(earnings)}],
        "total_equity_minor": equity_with_earnings, "total_equity": _fmt(equity_with_earnings),
...
async def get_audit_log(action: str | None = None, actor: str | None = None,
    ...
    if actor:
        ...
        params.append(actor)
    where = " AND ".join(conditions)
    count = await db.execute_fetchall(f"SELECT COUNT(*) AS cnt FROM audit_log WHERE {where}", params)
    rows = await db.execute_fetchall(
        f"SELECT * FROM audit_log WHERE {where} ORDER BY seq DESC LIMIT ? OFFSET ?",
        params + [limit, offset],
    )
    return {"items": [dict(r) for r in rows], "total": dict(count[0])["cnt"], "limit": limit, "offset": offset}
...
async def create_account(code: str, name: str, account_type: str,
                         description: str | None = None, actor: str = "mcp") -> dict:
    if account_type not in NORMAL_SIDE:
        raise ValueError(f"Invalid account type '{account_type}'. Must be one of: {', '.join(NORMAL_SIDE)}")
    db = await get_db()
    acct_id = _new_id("acc")
    now = _now()
    try:
        ...
        await _audit(db, actor, "create_account", "account", acct_id, f"Created account {code} · {name} ({account_type})")
        await db.commit()
...
async def post_transaction(txn_date: str, description: str, lines: list[dict],
                           reference: str | None = None, actor: str = "mcp",
                           source: str = "mcp") -> dict:
    ...
    try:
        ...
        raise ValueError(f"txn_date must be YYYY-MM-DD, got '{txn_date}'")
    ...
    await _audit(db, actor, "post_transaction", "transaction", txn_id,
...
async def transfer_funds(from_account: str, to_account: str, amount_minor: int,
                         txn_date: str, memo: str | None = None, actor: str = "mcp") -> dict:
    ...
    txn = await post_transaction(
        ...
        [
            {"account": src["id"], "direction": "credit", "amount_minor": amount_minor, "memo": memo},
            {"account": dst["id"], "direction": "debit", "amount_minor": amount_minor, "memo": memo},
        ],
        reference="TRF", actor=actor,
    )
    db = await get_db()
    await _audit(db, actor, "transfer_funds", "transaction", txn["id"],
...
async def reverse_transaction(txn_id: str, reason: str, actor: str = "mcp") -> dict:
    ...
    if original["status"] == "reversed":
        raise ValueError(f"Transaction {txn_id} is already reversed (by {original['reversed_by_id']})")
    ...
    await db.execute(
        ...
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (contra_id, today, f"Reversal of: {original['description']} — {reason}",
         original.get("reference"), txn_id, "mcp", now, actor),
    )
    lines = await db.execute_fetchall(
    ...
    for line in lines:
        ...
        await db.execute(
            "INSERT INTO entry_lines (id, transaction_id, account_id, line_no, direction, amount_minor, memo, created_at) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (_new_id("line"), contra_id, ln["account_id"], ln["line_no"], flipped, ln["amount_minor"], ln["memo"], now),
        )
    await db.execute("UPDATE transactions SET status = 'reversed', reversed_by_id = ? WHERE id = ?",
                     (contra_id, txn_id))
    await _audit(db, actor, "reverse_transaction", "transaction", contra_id,
