...
async def main():
    ...
    check(f"15 tools registered", len(tools) == 15, ", ".join(names))
    ...
    check("get_account (by name)", bank.get("code") == "1010", f"Bank balance {bank.get('balance')}")
    ...
    check("get_account_ledger", len(ledger["entries"]) >= 3,
    ...
    print("3. Write tools")
    acct = await call("create_account", code="5200", name="Office Supplies",
                      account_type="expense", description="smoke test")
    check("create_account", acct.get("normal_side") == "debit", f"{acct['code']} · {acct['name']}")

    bad = await call("post_transaction", txn_date=today, description="unbalanced test",
                     lines=[{"account": "Cash", "direction": "debit", "amount_minor": 100},
    ...
    posted = await call("post_transaction", txn_date=today, description="Bought office supplies (smoke test)",
                        reference="TEST",
                        lines=[{"account": "Office Supplies", "direction": "debit", "amount_minor": 4_500},
                               {"account": "Cash", "direction": "credit", "amount_minor": 4_500}])
    check("post_transaction (balanced)", posted.get("status") == "posted", f"{posted['id']} · {posted['total']}")

    moved = await call("transfer_funds", from_account="Wallet B", to_account="Wallet A",
                       amount_minor=10_000, txn_date=today, memo="smoke test")
    check("transfer_funds", moved.get("status") == "posted", f"{moved['description']} · {moved['total']}")

    contra = await call("reverse_transaction", transaction_id=posted["id"], reason="smoke test cleanup")
    original = await call("get_transaction", transaction_id=posted["id"])
    check("reverse_transaction", original["status"] == "reversed" and contra["reverses_id"] == posted["id"],
    ...
    check("trial balance STILL balanced after writes", tb2["balanced"],
          f"debits {tb2['total_debit']} == credits {tb2['total_credit']}")
    supplies = await call("get_balance", account="Office Supplies")
    check("reversal restored balance", supplies["balance_minor"] == 0,
